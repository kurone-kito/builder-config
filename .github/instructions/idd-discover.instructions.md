...
NOT contain a `builder-config-roadmap-id` marker (not itself
a roadmap) or a `builder-config-blocked-by` marker, AND
...
`builder-config-roadmap-id` marker.

**Legacy roots**: `--all-roadmaps` finds roots only by label or
`builder-config-roadmap-id` marker. Retro-label a legacy
...
`<!-- builder-config-roadmap-id: ... -->` marker are
...
- **A0-T only**: the scoped body-content lookup needed to resolve
  `builder-config-blocked-by` markers on the explicit target.
...
- **A0-O only** (when `issue-scope` is `orphan-first`, or when
  ...
  `builder-config-roadmap-id` or
  `builder-config-blocked-by` markers.
...
- **A3 only**: a body-content search (e.g.,
  ...
  `builder-config-roadmap-id` marker when checking
  `builder-config-blocked-by` dependency markers (see A3
...
- All dependency issues are closed or otherwise completed. Two forms:
  ...
  `<!-- builder-config-blocked-by: {roadmap-id} -->` markers —
  find the issue whose body contains a matching
  `<!-- builder-config-roadmap-id: {roadmap-id} -->`; treat as
...
`<!-- builder-config-autopilot-suitability: N -->` footer, or the
...
Read the authored `<!-- builder-config-effort: S|M|L -->` footer
...
- **Roadmap identity** (`builder-config-roadmap-id`): placed in
...
- **Sequential dependency** (`builder-config-blocked-by`): placed
...
**Do not use `builder-config-blocked-by` to group sub-tasks under
