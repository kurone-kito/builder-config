...
| External-check waiver comment (`idd-external-check-waiver`) | Human maintainer only, under the repository's `ciGate.externalCheckWaivers` policy — plus one narrow, documented exception: `github-actions[bot]`, for the `idd-advisory-convergence` selector's own `self-referential-bootstrap-auto` reason token only, when a PR's diff touches that check's own committed trigger-file allowlist (kurone-kito/idd-skill#2657). **Not wired in this repository**: `.github/workflows/idd-advisory-convergence.yml` runs with read-only `contents`/`issues`/`pull-requests` permissions, has no self-referential-bootstrap posting job, and carries no `pull_request_target` trigger, so `github-actions[bot]` cannot post this waiver here — treat that row as an opt-in upstream capability this repository has not adopted, not an available path | Not mutated once posted; superseded by a fresh waiver | F2/F3 CI gate (`waiverEvidence` checks); the bot-posted kind is additionally verified by resolving its `run-id:` field to a `pull_request_target`-triggered run of `idd-advisory-convergence.yml` for the current PR HEAD SHA |
