...
| Provider-degradation tuning         | Every field below absent — the `idd-advisory-convergence` required check polls the pre-#2333 hardcoded interval/budget, and the provider-outage-declaration and provider-health helpers ([idd-helper-scripts.md](idd-helper-scripts.md#provider-health-helper)) run on their built-in defaults | Three independent, optional containers in `.github/idd/config.json`: `advisoryConvergence.{copilotReviewPollInterval,copilotReviewPollMaxWait}` tune the required check's own short bounded poll for a pending primary-bot review; `providerOutage.{declarationTarget,maxValidity,maxParkedChanges}` name the issue read for an active outage-relief declaration and bound its validity window and parked-PR count; `providerHealth.{minCorroboratingPrs,samplingWindow}` tune the cross-PR `healthy`/`degraded`/`unavailable` classifier's corroboration floor and lookback window. None of the three gate merge or CI on their own -- see [Provider health helper](idd-helper-scripts.md#provider-health-helper), [Provider outage declaration helper](idd-helper-scripts.md#provider-outage-declaration-helper), and [Provider outage park helper](idd-helper-scripts.md#provider-outage-park-helper) for what actually consumes each. |
...
| Critique-pass telemetry hook        | Absent — no per-round C-phase telemetry is emitted                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Set `critiqueLoop.telemetryHook.command` in `.github/idd/config.json` (same `repository-local` / `user-global` / `none` resolution order as the delegate above; a repo-local `null` disables it outright and never inherits the user-global layer). Fire-and-forget: a missing command, non-zero exit, or timeout is silently ignored and never blocks a critique round. See [Repository-configurable critique telemetry hook](idd-workflow.md#repository-configurable-critique-telemetry-hook) and [Effective C-phase critique telemetry hook](idd-helper-scripts.md#effective-c-phase-critique-telemetry-hook).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Worktree guard base-branch refusal  | Absent or `false` (default) — the local git hook still allows a commit directly on `{development-branch}` in the primary worktree, matching the historical worktree-guard scope                                                                                                                                                                                                                                                                                                                                           | Set `worktreeGuard.refuseBaseBranchCommits: true` in `.github/idd/config.json` (`worktreeGuard.enabled` must also be `true`) to make the generated hook refuse a commit on `{development-branch}` in the primary worktree too, not only an `issue/*` / `roadmap-audit/*` implementation branch. The generated hook is pure POSIX sh with no network access, so it compares HEAD only against a repo-local, explicitly-configured `developmentBranch` — it cannot resolve the live GitHub default branch the way `idd-work.instructions.md`'s B1 does. Requires `developmentBranch` to be set; an absent value leaves this stricter check a no-op even with the opt-in enabled. Regenerate the hook after changing this value.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
