...
- **Cross-roadmap autopilot mode (`--all-roadmaps`)**: discovers every
  **open** roadmap root (an open issue carrying the `roadmap` label **or**
  an `<!-- builder-config-roadmap-id: ... -->` marker **or** a
  ...
  - **Opt-in leaf annotations** (additive; absent flags leave the leaf shape
    ...
    and hidden `builder-config-blocked-by` markers, plus
...
- **Legacy roots (`discover.legacyRoots`, #1315)**: a repository that
  ...
  label and the `builder-config-roadmap-id` marker, so they
...
| `disposition-threads` | At least one Copilot-authored review thread is unresolved and lacks a valid disposition marker (`threads.blockingCount > 0`). |
...
| `same-head-reroll` | A bounded same-HEAD reroll is currently requestable (`sameHeadReroll.requestable` is `true`; see AW6 above) -- its own eligibility conjunction (`sameHeadRerollTerms`) reuses `threadClause.satisfied` as one term, so it is mutually exclusive with `disposition-threads` specifically (both read `threads.blockingCount`); it can still co-occur with tokens whose conditions do not depend on that term, such as `disposition-posted-items`. |
